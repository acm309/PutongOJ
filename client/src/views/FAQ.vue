...
<template>
  <div class="faq-wrap">
    ...
    <Typography>
      <Title v-if="isZH" :level="4">
        问：各个评测状态的含义是什么？
      </Title>
      <Title v-else :level="4">
      ...
      <table>
        ...
        <tbody>
          <tr>
            <td class="info pd">
              <span v-if="isZH"><b>等待</b><br></span>Pending
            </td>
            <td v-if="isZH">
          ...
          <tr>
            <td class="info rj">
              <span v-if="isZH"><b>运行 & 评测</b><br></span>Running &amp; Judge
            </td>
            <td v-if="isZH">
              你的代码正在运行，并且正在被评测。
            </td>
          ...
          <tr>
            ...
            <td v-else>
              Your code cannot be compiled, usually because there are syntax errors in your code.
            </td>
          </tr>
          <tr>
            <td class="info ac">
          ...
          <tr>
            <td class="info wa">
              <b>{{ isZH ? '答案错误' : 'WA' }}</b><br>Wrong Answer
            </td>
            <td v-if="isZH">
              你的程序没有得到正确的结果。
            </td>
          ...
          <tr>
            <td class="info re">
              <b>{{ isZH ? '运行错误' : 'RE' }}</b><br>Runtime Error
            </td>
            <td v-if="isZH">
          ...
          <tr>
            <td class="info tle">
              <b>{{ isZH ? '时间超限' : 'TLE' }}</b><br>Time Limit Exceeded
            </td>
            <td v-if="isZH">
              你的程序运行时间超过了评测允许的时间。
            </td>
          ...
          <tr>
            <td class="info mle">
              <b>{{ isZH ? '内存超限' : 'MLE' }}</b><br>Memory Limit Exceeded
            </td>
            <td v-if="isZH">
              你的程序使用的内存超过了评测允许的内存。
            </td>
          ...
          <tr>
            <td class="info ole">
              <b>{{ isZH ? '输出超限' : 'OLE' }}</b><br>Output Limit Exceeded
            </td>
            <td v-if="isZH">
          ...
          <tr>
            <td class="info se">
              <b>{{ isZH ? '系统错误' : 'SE' }}</b><br>System Error
            </td>
            <td v-if="isZH">
              评测机发生了错误，这可能不是你的问题，请坐和放宽。
            </td>
            <td v-else>
              The judge encountered an error, which may not be your fault. Please relax.
            </td>
          </tr>
        </tbody>
      </table>
      ...
      <Title v-else :level="4">
        Q: What is the compiler / runtime environment of the judge?
      </Title>
      <Paragraph v-if="isZH">
        评测机的编译器 / 运行时环境如下：
      </Paragraph>
      ...
      <Title v-else :level="4">
        Q: Can my program know if it is running on the judge?
      </Title>
      <Paragraph v-if="isZH">
      ...
      <Title v-if="isZH" :level="4">
        问：为什么我得到了编译错误，但是在我的本地却可以编译通过？
      </Title>
      <Title v-else :level="4">
        Q: Why did I get a compile error, but it compiled successfully on my local machine?
      </Title>
      <Paragraph v-if="isZH">
      ...
      <Paragraph v-else>
        <ul>
          <li>
            <code>main</code> must be declared as <code>int</code>, <code>void main</code> will end up with a Compile
            Error.
          </li>
